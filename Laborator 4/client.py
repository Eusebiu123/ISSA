...
class Ui_MainWindow(object):
    ...
    def start_client(self):
        ...
        self.connected_msg.setText("Connected successfully")
    ...
    def recv_handler(self, stop_event):
        global server
        while True:
            message = cPickle.loads(server.recv(1024))
            print(message)
            self.set_dtc1_state(message)
            self.set_dtc2_state(message)
            self.set_dtc3_state(message)
            self.set_dtc4_state(message)
            self.set_led0_label(message)
            self.set_led1_label(message)
            self.set_led2_label(message)
            self.set_led3_label(message)
    ...
    def diag(self):
        global server, diag_ON, diag_mode
        server.send(cPickle.dumps(diag_ON))
        diag_mode = True
        self.diagMode.setEnabled(False)
        self.diagMode_off.setEnabled(True)

    def stop_diag(self):
        global server, diag_OFF, diag_mode
        server.send(cPickle.dumps(diag_OFF))
        diag_mode = False
        self.diagMode.setEnabled(True)
        self.diagMode_off.setEnabled(False)
    ...
    def set_led0_label(self, data_recv):
        global diag_mode, server
        if diag_mode:
            print("diag mod primul if", data_recv)
            self.led0_state.setVisible(True)
            if int(data_recv, 16) == int('0x6E011', 16):
                print("setez verde",data_recv)
                self.led0_state.setStyleSheet("background-color:red ;border-radius:5px;")
            elif int(data_recv, 16) == int('0x6E010', 16):
                print("setez rosu",data_recv)
                self.led0_state.setStyleSheet("background-color:green ;border-radius:5px;")

    def set_led0_flags(self):
        global diag_mode, server
        if diag_mode:
            if "Active" in self.dtc1_state.text():
                server.send(cPickle.dumps('0x2E011'))
            elif "Inactive" in self.dtc1_state.text():
                server.send(cPickle.dumps('0x2E010'))
    ...
    def set_led1_label(self, data_recv):
        ...
        if diag_mode:
            ...
            if int(data_recv, 16) == int('0x6E021', 16):
                print("setez verde", data_recv)
                self.led1_state.setStyleSheet("background-color:red ;border-radius:5px;")
            elif int(data_recv, 16) == int('0x6E020', 16):
                print("setez rosu", data_recv)
    ...
    def set_led1_flags(self):
        global diag_mode, server
        if diag_mode:
            if "Active" in self.dtc2_state.text():
                server.send(cPickle.dumps('0x2E021'))
            elif "Inactive" in self.dtc2_state.text():
                server.send(cPickle.dumps('0x2E020'))
    ...
    def set_led2_label(self, data_recv):
        ...
        if diag_mode:
            ...
            if int(data_recv, 16) == int('0x6E031', 16):
                print("setez verde", data_recv)
                self.led2_state.setStyleSheet("background-color:red ;border-radius:5px;")
            elif int(data_recv, 16) == int('0x6E030', 16):
                print("setez rosu", data_recv)
    ...
    def set_led2_flags(self):
        global diag_mode, server
        if diag_mode:
            if "Active" in self.dtc3_state.text():
                server.send(cPickle.dumps('0x2E031'))
            elif "Inactive" in self.dtc3_state.text():
                server.send(cPickle.dumps('0x2E030'))
    ...
    def set_led3_label(self, data_recv):
        ...
        if diag_mode:
            ...
            if int(data_recv, 16) == int('0x6E041', 16):
                print("setez verde", data_recv)
                self.led3_state.setStyleSheet("background-color:red ;border-radius:5px;")
            elif int(data_recv, 16) == int('0x6E040', 16):
                print("setez rosu", data_recv)
    ...
    def set_led3_flags(self):
        global diag_mode, server
        if diag_mode:
            if "Active" in self.dtc4_state.text():
                server.send(cPickle.dumps('0x2E041'))
            elif "Inactive" in self.dtc4_state.text():
                server.send(cPickle.dumps('0x2E040'))
