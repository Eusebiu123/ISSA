...
class Client:
    ...
    def __init__(self):
        ...
        print_system_notification("self.car_data = "+ str(self.car_data))
        if self.car_data['oil-pressure']<30 or self.car_data['oil-pressure']>=60:
            turn_oil_pressure_light(True)
        if self.car_data['FLtire-pressure']<=20 or self.car_data['FLtire-pressure']>=24:
            turn_tire_pressure_light(True)
        if self.car_data['FRtire-pressure'] <= 20 or self.car_data['FRtire-pressure'] >= 24:
            turn_tire_pressure_light(True)
        if self.car_data['RLtire-pressure'] <= 18 or self.car_data['RLtire-pressure'] >= 22:
            turn_tire_pressure_light(True)
        if self.car_data['RRtire-pressure'] <= 18 or self.car_data['RRtire-pressure'] >= 22:
            turn_tire_pressure_light(True)
        if self.car_data['engine-temperature']>90:
            turn_engine_temperature_light(True)
        if self.car_data['battery-level']<15:
    ...
    def handle_message(self, command):
        if command.startswith('display-popup'):
            ...
            if len(command.split('#'))>=2:
                 self.print_popup(command.split('#')[1])
